...
class FourierFeatures(nn.Module):
    def __init__(self, in_features, out_features, std=1.):
        ...
        self.weight = nn.Parameter(torch.randn([out_features // 2, in_features]) * std).to(torch.device('mps'))
...
class DownSample(nn.Module):
    def __init__(self):
        ...
        self.down_2 = nn.Sequential(
            ...
            SelfAttention2d(c * 4, c * 4 // c),
            ResConvBlock(c * 4, c * 4, c * 4),
            SelfAttention2d(c * 4, c * 4 // c),
        )

    def forward(self, input):
        down_sample = self.avg_0(self.down_0(input))
...
class MidSample(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Conv2d(8*c, 4*c, kernel_size = 3, padding = 1);

        self.mid_1 = nn.Sequential(
            ResConvBlock(c * 4, c * 8, c * 8),
            SelfAttention2d(c * 8, c * 8 // c),
...
class UpSample(nn.Module):
    def __init__(self):
        super().__init__()
        c = 64
        self.conv_0 = nn.Conv2d(2*c, c, kernel_size = 3, padding = 1)
        self.conv_1 = nn.Conv2d(4*c, 2*c, kernel_size = 3, padding = 1)
        self.conv_2 = nn.Conv2d(8*c, 4*c, kernel_size = 3, padding = 1)
...
class Diffusion(nn.Module):
    ...
    def forward(self, input, t, cond, type = "Source"):
        ...
        # print('class_embed', t.shape, timestep_embed.shape, cond.shape, class_embed.shape)
...
# df.forward(torch.zeros(8, 6, 128, 256).to(device), torch.zeros(8).to(device), torch.zeros(8).long().to(device))
